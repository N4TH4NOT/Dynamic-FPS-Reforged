...
public final class Config {
    ...
    public int frameRateTarget() {
        if (this.frameRateTarget < 0) return this.frameRateTarget;
        else return Constants.NO_FRAME_RATE_LIMIT;
    }

    public void setFrameRateTarget(int value) {
        if (value == Constants.NO_FRAME_RATE_LIMIT) this.frameRateTarget = -1;
        else this.frameRateTarget = value;
    }

    public float volumeMultiplier(SoundSource source) {
        if (this.rawVolumeMultiplier(SoundSource.MASTER) == 0.0f) return 0.0f;
        else return this.rawVolumeMultiplier(source);
    }
    ...
    public void setVolumeMultiplier(SoundSource source, float value) {
        ...
        if (value != 1.0f) this.volumeMultipliers.put(key, value);
        else this.volumeMultipliers.remove(key); // Same as default value
    }
    ...
    public static Config getDefault(PowerState state) {
        return switch (state) {
            case HOVERED -> new Config(60, withMasterVolume(1.0f), GraphicsState.DEFAULT, true, false);
            case UNFOCUSED -> new Config(1, withMasterVolume(0.25f), GraphicsState.DEFAULT, false, false);
            case ABANDONED -> new Config(10, withMasterVolume(1.0f), GraphicsState.DEFAULT, false, false);
            case INVISIBLE -> new Config(0, withMasterVolume(0.0f), GraphicsState.DEFAULT, false, false);
            default -> throw new RuntimeException("Getting default configuration for unhandled power state " + state);
        };
    }
